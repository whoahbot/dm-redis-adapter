...
Benchmark.bm(50) do |x|
  ...
  x.report("Create 1000 posts with RedisClient") do 
    1000.times do
...
Benchmark.bm(50) do |x|
  ...
  x.report("Fetch the 500th post's text with RedisClient") { redis.set_member?("Post:id:all", 500); redis["Post:500:text"] }
end
...
Benchmark.bm(50) do |x|
  ...
  x.report("Fetch the text of all posts with RedisClient") { 1000.times {|n| redis["Post:#{n}:text"]} }
end
